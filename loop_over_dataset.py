...
# configs_det.save_results = True # True = save groundtruth labels as objects, False = save model-based detection
...
exec_data = ['pcl_from_rangeimage',] # options are 'load_image', 'pcl from rangeimgae' 
...
while True:
    try:
        ...
        if 'measure_detection_performance' in exec_list:
            ...
            # save_object_to_file(
            #     object=det_performance,
            #     file_path=results_fullpath,
            #     base_filename=data_filename,
            #     object_name='det_performance_' + configs_det.arch + '_' + str(configs_det.conf_thresh),
            #     frame_id=cnt_frame
            # )    
        else:
